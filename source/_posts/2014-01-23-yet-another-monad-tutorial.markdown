...
Functional programming has become popular these days, but unlike object-oriented languages, each FP language is so different from each other. Some of these use strict evaluation while others use lazily evaluated models; tons of new concurrent models were introduced; further more, states are handled differently too.

Haskell, for example, does not have states, but uses its powerful type system to construct the stateful program flow normally used in other languages. As you might guess, Monad is one of the type that does the trick. Defining a Monad type is pretty much like you would do to define a class in an OO language. However, Monad can do much more than a class. It's a type that can be used for exception handling, constructing parallel program workflow or even a parser generator!
...
Before we jump into Monad itself, let's quickly go through the type systems. What is a type? We usually think of type as a static thing like `int` or `float`, but actually, type is more dynamic; **types are the rules associated with the data, not the actual data itself**.

For example, an `int` is treated as `int` only when you use `addl` CPU instruction to present `+` operation on 64 bit machine. On the other hand, `float` uses `addss` instruction for `+` and `%xmm` registers for computations. It's the *rules*, or the *generated instructions*, that define the type.

The whole concept of Object-Oriented Programming is to let you to use classes/interfaces to define types, the rules (methods) that interacts with the actual data. For more advanced needs, people use templates(c++) or generics(java) to define more abstracted rules that are more reusable. And Monad, is pretty much like that generic class.
...
By going through these four rules and looking at the examples below, you'll understand what is a Monad and why is it so powerful!

Monad lesson 1: the bind operator `(>>=)`
----------------------------------------

The definition of Monad bind operator is a function type signature. A type signature is like an abstract method defined in abstract class:
...
2. A function `(a -> m b)` as the second input. `(a -> m b)` is a first order function that takes `a` as input, and returns Monad `m b`. You can think this as a delegate design pattern in Object-Oriented world, except you don't need to pass in a whole object, but only a function itself is sufficient.
...
Why is this definition powerful? Well, one benefit is that you can wrap common logic in Monad and safe your time.

* * *

## Application 1: exception handling

Exception handling is always a pain ass. Lets demonstrate how to use a Monad to reduce the code size and un-nest if-else blocks.
...
When you combine several Maybe Monad handling functions together, if one of the upstream function went wrong by returning `Nothing`, all the downstream function won't be executed.
...
1. Error handling if-else logic is encapsulated from user.
...
In object-oriented kindom, we use objects to encapsulate error messages, throw it out to try-catch block.
...
So far, we've seen that Monad can help us to wrap common (usually nested) logic into a type.
User can **lift** the data to the upward Monad, and Monad do the if-else logic for you.
Nevertheless, Monad can do more for you! `Maybe` Monad only pass the data to the next
...
Don't be scared! Writer Monad use `Monoid` to concat the data you lift to it.
`Monoid` defines `mempty` and `mappend` functions. `mappend`, as it name suggests, append
the data you passed to it; and `mempty` is just an empty element.
`mappend` and `mempty` are just an abstract way to merge data together.
...
The Writer Monad's definition just simply takes the return value of user defined function `f`, extract
the value `v'` out of it, and use `mappend` to append the old `v` and new `v'`.

#### Writer Monad, in action

Let me take a modified example from [Learn You a Haskell for Great good][]. To log the process of deriving greatest common divisor, we can do this:
...
The Writer Monad don't have a constructor like Maybe Monad does, so you need to use `tell` and `return` to build the Writer Monad and return it back. To show the result of the writer log, you'll also need function `runWriter`.
...
So, now we learned you can put different logic in a Monad to operate states. All the state passing is done by bind operator `>>=`, which wraps the data and pass to the downstream handler. However, Sometimes, we don't care the wrapped value, but just want to pass the state downstream. For example, performing side effects.
...
What is the use case of this? Recall that Haskell is a purely functional language that doesn't has states or variables; However, the Monad system can encapsulate hidden states, so the solution is put a hidden `#realworld#` variable in `IO Monad` that record the standard input-output status, and also the file seek position, etc.
...
`fail` can take an additional string to report the failure message. With *bind*, *then*, *return*, and *fali* functions, now we know the whole definition of the Monad type!

Monad Lesson 4: Beyond states
-----------------------------

So far, we introduced `Maybe`, `Writer`, and `IO ()` Monads which use states heavily, but Monad can do more than abstracting states. Remember, the core of Monad type is to wrap around logic. One of the applications it can do is defining work flow. Here comes the [Eval Monad][] for the rescue; let's take a look on the example in the online book:
...
Once you wrap the function `f` in `Eval` Monad, it generates the logic to evaluate the code in parallel. You can further use the combination of `rpar` and `rseq` function defined by `Eval`, to construct the dependencies of each individual jobs. Check the link for more details!
...
I started to learn Haskell a while ago, but I found explaining what a Monad type is in a easy way is extremely difficult. The final approach I choose is breaking it down to four functions and explore them by examples. 
I found it helps me to understand it better, and I hope it helps you out as well.
